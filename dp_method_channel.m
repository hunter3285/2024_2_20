function [sum_rate_optimal_dp, visited_dp, visited_temp, visited_temp_2, all_step_dp, last_step_matrix, dp_matrix, n_grid, rate_vec, dp_inst]=...
    dp_method_channel(dp_inst)
...
visited_temp=zeros(dp_inst.N_cell_x, dp_inst.N_cell_y);
visited_temp_2=visited_temp;
for ii=1:dp_inst.N_cell_x
    for jj=1:dp_inst.N_cell_y
        visited_temp(ii,jj)=dp_inst.visited_cells_matrix(x,y,direction+1, time+1, (jj-1)*10+ii);
        visited_temp_2(ii,jj)=dp_inst.visited_cells_matrix_2(x,y,direction+1, time+1, (jj-1)*10+ii);
    end
end
n_grid=sum(visited_temp.*dp_inst.sensing_matrix, "all")+ sum(visited_temp_2.*dp_inst.sensing_matrix_2, "all");
