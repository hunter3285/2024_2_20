...
Ts=obj.Ts;
N_azi=obj.N_azi;
UAV_start_end=obj.UAV_start_end;
H=obj.H;
c=3e8;
M=obj.M;
N=obj.N;
...
for ii=1:N_steps
    ...
    current_middle=obj.get_current_middle(current_point(1), current_point(2));
...
%     vr=cell_side/N_azi/T_PRI;
    ...
    N_points=obj.N_scatterer_matrix(current_point(1), current_point(2));
    ...
    points=obj.image_cell_matrix(current_point(1), current_point(2)).scatterer_pos;
    for jj=1:N_points
...
%             ones(size(C,1),1).*mean(C_cols,2);
    ...
    if direction==3
        C_new=rot90(C_new);
        obj.image_cell_matrix(current_point(1), current_point(2)).image=C_new;
        obj.image_cell_matrix(current_point(1), current_point(2)).image_x=x;
        obj.image_cell_matrix(current_point(1), current_point(2)).image_y=y;
    elseif direction==0
        C_new=rot90(C_new,2);
        obj.image_cell_matrix(current_point(1), current_point(2)).image_horizontal=C_new;
        obj.image_cell_matrix(current_point(1), current_point(2)).image_x_horizontal=x;
        obj.image_cell_matrix(current_point(1), current_point(2)).image_y_horizontal=y;
    elseif direction==2
        % C_new=rot90(C_new,2);
        obj.image_cell_matrix(current_point(1), current_point(2)).image_horizontal=C_new;
        obj.image_cell_matrix(current_point(1), current_point(2)).image_x_horizontal=x;
